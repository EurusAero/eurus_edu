...
- [Яндекс.Диск с актуальным ПО](https://disk.yandex.ru/d/4tVH5i1zkaNgpw)
...
6. Подключите Orange Pi к компьютеру:
    ...
    - С зажатой кнопкой Maskrom подайте питание на Orange Pi
        ...
        ВНИМАНИЕ: Стандартные USB-порты компьютеров и ноутбуков выдают ток всего 0.5А – 0.9А. Для полноценной работы Orange Pi 5B этого может быть недостаточно. На этапе прошивки в режиме Maskrom этого тока может хватить, но для последующего запуска системы, в случае отсутствия обратной реакции от платы, её рекомендуется переключить на внешний блок питания.
...
2. Подключите питание к Orange Pi 5B.
    ...
    ВАЖНОЕ ТРЕБОВАНИЕ К ПИТАНИЮ: Для работы платформы используйте сетевой блок питания, выдающий 5V / 4A (минимум 5V / 3A). Из-за ограничения портов ноутбука в 0.9А плате не будет хватать питания. Если входящего тока недостаточно, Orange Pi 5B начнет циклически перезагружаться, зависнет или не сможет запустить операционную систему
