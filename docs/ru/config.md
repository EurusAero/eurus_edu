...
- `buffer_size` - Размер буффера api сервера
...
- aruco_borders_kp - Коофициент увеличения скорости возврата в допустимую область
...
- `max_travel_distance_per_command` - Максимальное расстояние, которое дрон может пройти за одну команду перемещения (в метрах)
...
Поскольку расстояние превышает установленное ограничение, команда завершится ошибкой:

`Превышена максимальная дистанция перемещения за команду`
