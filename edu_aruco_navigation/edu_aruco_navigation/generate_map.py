...
def main():
    ...
    parser.add_argument("-bl", "--bottom_left", action="store_true", help="Сделать (0,0) в левом нижнем углу (нумерация пойдет сверху вниз)")
    ...
    with open(args.out, mode='w', newline='', encoding='utf-8') as f:
        ...
        for y_idx in range(args.cy):
            for x_idx in range(args.cx):
                ...
                writer.writerow([current_id, f"{args.l:.4f}", f"{x:.4f}", f"{y:.4f}", f"{z:.4f}"])
                current_id += 1

    print(f"Сгенерирована карта {args.cx}x{args.cy} ({args.cx * args.cy} маркеров).")
    if args.bottom_left:
        print("Режим '-bl': Координата (0,0) установлена в левом нижнем углу.")
