...
class EurusControl:
    ...
    def connect(self):
        ...
        try:
            ...
            self.listener_thread = threading.Thread(
                target=self._listen_server, daemon=True
            )
            self.listener_thread.start()

            self.heartbeat_thread = threading.Thread(
                target=self._heartbeat_server, daemon=True
            )
    ...
    def _heartbeat_server(self):
        while self.running:
            try:
                ...
                payload = {"command": "heartbeat", "timestamp": time.time()}
                ...
                for raw_msg in messages:
                    ...
                    try:
                        ...
                        if command == "response":
                            if self.do_log:
                                self.logger.info(
                                    f" - Получен ответ: {msg_dict.get('status')}"
                                )
                            ...
                            if action_name == "laser_shot":
                                if self.do_log:
                                    self.logger.info(
                                        f" - Получен статус от лазера: {code}"
                                    )
                                ...
                                if self.do_log:
                                    self.logger.info(
                                        f" - Получен статус: {command} ({action_name}): {code} ({self._last_action_message})"
                                    )
                            ...
                            self._last_point_reached_data = msg_dict.get(
                                "point_reached", {}
                            )
                            self._point_reached_event.set()
            ...
            try:
                if self.sock:
                    ...
                    if payload["command"] in DRONE_COMMANDS and self.do_log:
                        self.logger.info(
                            f" - Отправлена комманда: {payload['command']}"
                        )
            except Exception as e:
                if self.do_log:
    ...
    def _smart_wait(
        self, event: threading.Event, timeout: float = None, error_msg: str = None
    ) -> bool:
        ...
        try:
            with self._movement_lock:
                ...
                if not self._smart_wait(
                    self._response_event, 30.0, "ТАЙМ-АУТ: Нет ответа от сервера!"
                ):
                    return False
                ...
                if not self._smart_wait(
                    self._action_started_event,
                    10.0,
                    f"Команда {cmd_name} не перешла в PENDING",
                ):
                    return False

                if (
                    self._action_finished_event.is_set()
                    and self._last_action_code == DENIED_STATUS
                ):
                    if self.do_log:
                        self.logger.error(
                            f"Команда {cmd_name} отклонена: {self._last_action_message}"
                        )
                    return False
                ...
                if not self._smart_wait(self._action_finished_event, timeout=None):
                    if self.do_log:
                        self.logger.warning(
                            "Ожидание завершения прервано (отключение)."
                        )
                    return False
                ...
                if self._last_action_code == COMPLETED_STATUS:
                    ...
                    if self.do_log:
                        self.logger.error(
                            f"Команда {cmd_name} провалена (Status: {self._last_action_code}). Msg: {self._last_action_message}"
                        )
                    self.disconnect()
                return False
    ...
    def takeoff(self, altitude, speed=0.1):
        return self._send_movement_command(
            {"command": "takeoff", "altitude": float(altitude), "speed": float(speed)}
        )
    ...
    def laser_shot(self):
        ...
        if self._smart_wait(self._laser_event, timeout=2.0):
            if self._last_laser_status == COMPLETED_STATUS:
                ...
                if self.do_log:
                    self.logger.warning(
                        f"Выстрел лазером не удался/отменён: {self._last_laser_status}"
                    )
                return False
        else:
            if self.do_log:
                self.logger.error(
                    "Превышено время ожидания для выстрела лазером (нет подтверждения от сервера)."
                )
            return False
    ...
    def aruco_map_navigation(self, state=False, fly_in_borders=True):
        ...
        payload = {
            ...
            "state": state,
            "fly_in_borders": fly_in_borders,
        }

        self._send_raw(payload)

    def move_to_marker(
        self, marker_id: str | int, z: float, speed: float = 1.0, yaw: float = None
    ):
        if not self.is_connected:
            if self.do_log:
                self.logger.warning("Нет соединения для отправки команды.")
