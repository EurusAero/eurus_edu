...
class MavrosHandler(Node):
    def __init__(self):
        ...
        self.point_reached_pub = self.create_publisher(
            Bool, "/edu/point_reached", qos_profile
        )
        ...
        self.create_subscription(Command, "edu/command", self.command_callback, 10)
        ...
        self.create_subscription(
            State, "/mavros/state", self.state_updater, mavros_qos_profile
        )

        self.create_subscription(
            String, "/edu/aruco_map_nav", self.aruco_nav_updater, mavros_qos_profile
        )
        ...
        if os.path.exists(configuration_path):
            ...
            self.max_yaw_rate = float(
                restrictions_config["restrictions"].get(
                    "max_yaw_rate", self.max_yaw_rate
                )
            )
            self.max_travel_distance_per_command = float(
                restrictions_config["restrictions"].get(
                    "max_travel_distance_per_command",
                    self.max_travel_distance_per_command,
                )
            )
        else:
            self.get_logger().warn(
                f"Не обнаружен конфигурационный файл {configuration_path} используются значения по умолчанию."
            )
        ...
        if self.aruco_map_path and os.path.exists(self.aruco_map_path):
            ...
            self.get_logger().warn("Аруко карта не создана.")
        ...
        if os.path.exists(handler_ini_path):
            ...
            self.max_corr = config.getfloat(
                "velocity", "aruco_borders_correlation_speed"
            )
            self.aruco_border_indent = config.getfloat(
                "velocity", "aruco_border_indent"
            )

            self.point_reached_diff = config.getfloat(
                "local_position", "point_reached_diff"
            )
    ...
    def cmd_loop(self):
        ...
        if self.current_control_method == "LOCAL_POSITION":
            ...
            self.point_reached.data = (
                self.get_distance(self.local_pose, self.target_pose)
                < self.point_reached_diff
            )
    ...
    def publish_status(self, original_msg, status, message=""):
        try:
            ...
            self.get_logger().info(
                f"Статус '{original_msg.command}': {status} | {message}"
            )
        except Exception as e:
            self.get_logger().error(f"Ошибка при публикации статуса: {e}")
    ...
    def calculate_next_target_position(self):
        ...
        if aruco_active and not map_visible and (time.time() - last_seen_ts) > 0.5:
            ...
            hold_pos = True
            self.get_logger().warn("Карта потеряна, удержание позиции.")

        stamp = self.start_position.header.stamp
        total_dist = self.get_distance(self.start_position, self.target_pose)

        flight_duration = (
            total_dist / self.setpoint_speed if self.setpoint_speed > 0 else 0
        )
        ...
        if flight_duration > 0 and not hold_pos:
            ...
            self.setpoint_pose.pose.position.x = (
                self.start_position.pose.position.x
                + (
            ...
            self.setpoint_pose.pose.position.y = (
                self.start_position.pose.position.y
                + (
            ...
            self.setpoint_pose.pose.position.z = (
                self.start_position.pose.position.z
                + (
    ...
    def calculate_next_target_velocity(self):
        ...
        if aruco_active:
            if (
                not map_visible
                and (time.time() - last_seen_ts) > 0.5
                and not self.hold_zero_velocity
            ):
                local_vx = 0.0
                local_vy = 0.0
                ...
                if (
                    local_vx > 0
                    and local_x > self.map_width_max - self.aruco_border_indent
                ):
                    local_vx = 0.0
                ...
                if (
                    local_vx < 0
                    and local_x < self.map_width_min + self.aruco_border_indent
                ):
                    local_vx = 0.0
                ...
                if (
                    local_vy > 0
                    and local_y > self.map_height_max - self.aruco_border_indent
                ):
                    local_vy = 0.0
                ...
                if (
                    local_vy < 0
                    and local_y < self.map_height_min + self.aruco_border_indent
                ):
                    local_vy = 0.0
    ...
    def set_zero_velocity(self, pos_x=None, pos_y=None):
        self.get_logger().debug("Установка нулевой скорости.")
    ...
    def command_callback(self, msg: Command):
        ...
        self.current_task_thread = threading.Thread(
            target=self.execute_command_logic, args=(msg,), daemon=True
        )
    ...
    def execute_command_logic(self, msg: Command):
        ...
        try:
            ...
            self.get_logger().warn("Ошибка при обработке JSON сообщения")
            return
    ...
    def set_home_position(self, save_altitude=False):
        ...
        self.get_logger().info(
            f"home position установлена в: x: {self.home_position.pose.position.x}, y: {self.home_position.pose.position.y}, orientation: {self.home_position.pose.orientation}"
        )
    ...
    def do_set_mode(self, mode="OFFBOARD"):
        ...
        self.get_logger().warn(
            f"Не удалось отправить команду установки режима {mode}: {res.result}"
        )
    ...
    def do_arm(self):
        ...
        self.get_logger().info(
            f"Отправлена команда Arm результат: {'Успех' if res else 'Провал'}"
        )
        if res.success:
            return True, "Armed"
    ...
    def do_disarm(self):
        ...
        res = self._call_service_sync(self.arming_client, req)
        self.get_logger().info(
            f"Отправлена команда Disarm результат: {'Успех' if res else 'Провал'}"
        )
        if res.success:
            return True, "Disarmed"
        return False, "Disarm неудался"
    ...
    def do_land(self):
        ...
        self.get_logger().info(
            f"Отправлена команда на посадку результат: {'Успех' if res else 'Провал'}"
        )
        if res.success:
            return True, "Посадка..."
        ...
        try:
            ...
            if yaw_rate is not None:
                if (
                    ...
                    and abs(yaw_rate) <= self.max_yaw_rate
                    and self.max_yaw_rate > 0
                ):
                    ...
                    return (
                        False,
                        f"Превышена максимальная скорость поворота {yaw_rate} из {self.max_yaw_rate}",
                    )
            ...
            return (
                True,
                f"Устанавливается vx={vx}, vy={vy}, vz={vz}, yaw_rate={yaw_rate}rad",
            )
    ...
    def do_move_to_local_point(self, data, ignore_restrictions=False):
        try:
            ...
            if (
                not ignore_restrictions
                and dist_diff > self.max_travel_distance_per_command
                and self.max_travel_distance_per_command > 0
            ):
                return (
                    False,
                    f"Превышена максимальная дистанция перемещения за команду {dist_diff} из {self.max_travel_distance_per_command}",
                )
            ...
            if self.aruco_nav_status.get("aruco_nav_status"):
                ...
                self.target_pose.pose.position.x = (
                    self.home_position.pose.position.x + x
                )
                self.target_pose.pose.position.y = (
                    self.home_position.pose.position.y + y
                )
                self.target_pose.pose.position.z = z
            ...
            if yaw is not None:
                ...
                self.setpoint_pose.pose.orientation.x = (
                    self.local_pose.pose.orientation.x
                )
                self.setpoint_pose.pose.orientation.y = (
                    self.local_pose.pose.orientation.y
                )
                self.setpoint_pose.pose.orientation.z = (
                    self.local_pose.pose.orientation.z
                )
                self.setpoint_pose.pose.orientation.w = (
    ...
    def do_move_in_body_frame(self, data, ignore_restrictions=False):
        try:
            ...
            if self.aruco_nav_status.get("aruco_nav_status"):
                if self.aruco_nav_status.get(
                    "map_in_vision"
                ) and self.aruco_nav_status.get("fly_in_borders"):
                    x = max(self.map_width_min, min(x, self.map_width_max))
                    y = max(self.map_height_min, min(y, self.map_height_max))
                else:
                    return False, "В зоне видимости нет аруко маркеров"
            ...
            return (
                True,
                f"Перемещение в x={self.target_pose.pose.position.x}, \
        ...
        try:
            if self.aruco_nav_status.get("aruco_nav_status"):
                ...
                if self.aruco_nav_status.get("map_in_vision"):
                    ...
                    return self.do_move_to_local_point(
                        setpoint_data, ignore_restrictions=True
                    )
                else:
                    return False, "В зоне видимости нет аруко маркеров"
            else:
